...
class OeLexwareExportImex extends oxBase
{
     ...
     *
     * @param int $iFromOrderNr Order from (default null)
     * @param int $iToOrderNr   Order number
     *
     * @return string
     */
    public function exportLexwareOrders($iFromOrderNr = "", $iToOrderNr = "")
    {
        ...
        foreach ($oOrderlist as $oOrder) {
            ...
            foreach ($oOrderArticles as $oOrderArt) {
                $dVATSet = array_search($oOrderArt->oxorderarticles__oxvat->value, $myConfig->getConfigParam('aOELexwareExportVAT'));
